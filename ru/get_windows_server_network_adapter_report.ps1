<#
.SYNOPSIS
    Собирает сведения о сетевых адаптерах Windows Server из списка серверов в TXT-файле.
.DESCRIPTION
    Скрипт читает список серверов из текстового файла и подключается к ним через WMI/DCOM.
    Такой способ совместим с Windows Server 2008-2025 и не зависит от наличия современных
    модулей NetAdapter/NetTCPIP на целевой системе.

    Поддерживается многопоточный режим через фоновые jobs. Если скрипт запущен из PowerShell 7+,
    для WMI-сбора автоматически используется Windows PowerShell 5.1 worker-процесс.

    Скрипт собирает:
    - список текущих сетевых адаптеров и их параметры;
    - IP-адреса, DNS, DHCP и шлюзы;
    - default gateway из конфигурации адаптеров и из IPv4 route table;
    - старые/неактуальные сетевые профили из реестра и class registry.

    При запуске без параметров учётных данных скрипт интерактивно предлагает:
    - использовать текущего пользователя;
    - ввести другие учётные данные.
.PARAMETER ComputerListPath
    Путь к TXT-файлу со списком серверов, по одному имени на строку.
.PARAMETER OutputDirectory
    Папка для сохранения результатов. По умолчанию создаётся output\<дата_и_время>.
.PARAMETER Credential
    Учётные данные для удалённого подключения по WMI. Если параметр не указан,
    скрипт предложит выбрать текущего пользователя или ввести другие учётные данные.
.PARAMETER UseCurrentUser
    Запускать без запроса учётных данных, используя текущего пользователя.
.PARAMETER Parallel
    Обрабатывать список серверов параллельно через фоновые jobs.
.PARAMETER ThrottleLimit
    Максимальное число одновременно работающих jobs в параллельном режиме.
.EXAMPLE
...
if ($Credential -and $UseCurrentUser.IsPresent) {
    throw 'Параметры -Credential и -UseCurrentUser нельзя использовать одновременно.'
}

if ($InternalWorker.IsPresent -and [string]::IsNullOrWhiteSpace($InternalComputerName)) {
    throw 'Во внутреннем worker-режиме должен быть указан параметр -InternalComputerName.'
}
...
if ($InternalWorker.IsPresent -and -not $script:HasGetWmiObject) {
    throw 'Во внутреннем worker-режиме требуется Windows PowerShell с командой Get-WmiObject.'
}
...
function Get-LegacyDetectionLabel {
    ...
    switch ($DetectionType) {
        'LegacyWithTcpipProfile' { return 'Старый адаптер с сохранённым TCP/IP-профилем' }
        'RegistryOnly' { return 'Остался только TCP/IP-профиль в реестре' }
        'ClassOnly' { return 'Осталась только запись драйвера/адаптера' }
...
function Get-LegacyDetectionDescription {
    ...
    switch ($DetectionType) {
        'LegacyWithTcpipProfile' { return 'Адаптер уже не активен, но в системе остались и драйвер, и сетевые настройки.' }
        'RegistryOnly' { return 'Адаптер больше не найден, но TCP/IP-настройки ещё сохранены в ветке Interfaces.' }
        'ClassOnly' { return 'Сохранилась запись адаптера в class registry, но сетевой TCP/IP-профиль уже отсутствует.' }
        default { return $null }
    }
}
...
function Get-LegacyAddressingMode {
    ...
    if (-not [string]::IsNullOrWhiteSpace($StaticIPAddress)) {
        return 'Статическая конфигурация'
    }
    ...
    return 'Нет сохранённых сетевых настроек'
}
...
function Get-ServerListFromFile {
    ...
    if (-not (Test-Path -LiteralPath $resolvedListPath)) {
        ...
        if (Test-Path -LiteralPath $examplePath) {
            throw "Файл со списком серверов не найден: $resolvedListPath. Используйте шаблон: $examplePath"
        }

        throw "Файл со списком серверов не найден: $resolvedListPath"
    }
    ...
    if ($servers.Count -eq 0) {
        throw "Файл '$resolvedListPath' не содержит серверов для обработки."
    }
...
function Resolve-AuthenticationContext {
    ...
    if ($ProvidedCredential) {
        return [PSCustomObject]@{
            ...
            Mode = 'Credential'
            DisplayName = 'Указанные учётные данные'
        }
    }
    ...
    Write-Host 'Выберите режим подключения к серверам:' -ForegroundColor Yellow
    Write-Host '1. Использовать текущего пользователя' -ForegroundColor DarkYellow
    Write-Host '2. Ввести другие учётные данные' -ForegroundColor DarkYellow
    ...
    while ($true) {
        ...
        switch ($choice) {
            ...
            '2' {
                return [PSCustomObject]@{
                    Credential = Get-Credential -Message 'Введите учётные данные для удалённого WMI-подключения'
                    Mode = 'Credential'
                    DisplayName = 'Запрошенные учётные данные'
                }
            }
            default {
                Write-Warning 'Нужно выбрать 1 или 2.'
            }
        }
    }
}
...
function Get-WorkerPowerShellExecutablePath {
    ...
    throw 'Не удалось определить PowerShell-движок для WMI-сбора. Требуется Windows PowerShell 5.1.'
}
...
function Convert-WorkerExecutionToResult {
    ...
    if ($ExitCode -eq 0 -and -not [string]::IsNullOrWhiteSpace($jsonText)) {
        try {
            ...
            $parseErrorMessage = 'Не удалось разобрать результат worker-процесса: {0}' -f $_.Exception.Message
    ...
    $errorText = if (-not [string]::IsNullOrWhiteSpace($jsonText)) {
        ...
        'Worker-процесс завершился без результата. Код возврата: {0}' -f $ExitCode
    }
    ...
    foreach ($job in $completedJobs) {
        ...
        if ($null -ne $structuredOutput -and $structuredOutput.PSObject.Properties['TargetComputerName']) {
            ...
            $fallbackErrorText = if ($job.ChildJobs.Count -gt 0 -and $job.ChildJobs[0].Error.Count -gt 0) {
                ...
                'Фоновая job завершилась без данных.'
            }
    ...
    try {
        if (-not $UseParallel.IsPresent) {
            ...
            foreach ($server in $ServerList) {
                Write-Host "Обработка: $server" -ForegroundColor Cyan
        ...
        if (-not (Get-Command -Name Start-Job -ErrorAction SilentlyContinue)) {
            Write-Warning 'Команда Start-Job недоступна. Используется последовательный режим.'
        ...
        if ($PSVersionTable.PSVersion.Major -lt 5) {
            Write-Warning 'Для стабильного параллельного режима рекомендуется PowerShell 5.1 или новее. Используется последовательный режим.'
            return Invoke-NetworkInventoryCollection -ServerList $ServerList -AuthenticationContext $AuthenticationContext -MaxParallelJobs $MaxParallelJobs
        }

        Write-Host ("Параллельный режим включён. Одновременно обрабатывается до {0} серверов." -f $MaxParallelJobs) -ForegroundColor DarkGreen
        ...
        foreach ($server in $ServerList) {
            ...
            Write-Host "Запуск job для: $server" -ForegroundColor Cyan
    ...
    try {
        ...
        if ($configuredGatewayAssignments.Count -gt 1 -or $uniqueRouteGatewayValues.Count -gt 1) {
            ...
            $issueText = if ($configuredGatewayAssignments.Count -gt 1) {
                'На сервере найдено несколько default gateway в настройках сетевых адаптеров.'
            }
            else {
                'В таблице маршрутов найдено несколько default route.'
            }
            ...
            $multipleGatewayAlerts = @([PSCustomObject]@{
                ...
                Issue = $issueText
                Recommendation = 'Проверьте, действительно ли сервер должен использовать несколько шлюзов по умолчанию.'
            })
        }
        ...
        $legacyRows = @($legacyRows | ForEach-Object {
            ...
            $adapterDisplayName = Get-FirstNonEmptyString -Values @($_.AdapterName, $_.Manufacturer, 'Не удалось определить адаптер')
            $registryLocations = @()
            ...
            $savedSettingsSummary = Join-Items -Value @(
                ...
                if ($savedSubnetMask) { 'Маска: {0}' -f $savedSubnetMask }
                if ($savedGateway) { 'Шлюз: {0}' -f $savedGateway }
                if ($savedDnsServers) { 'DNS: {0}' -f $savedDnsServers }
                if ($savedDomain) { 'DNS-суффикс: {0}' -f $savedDomain }
            )
...
function Invoke-NetworkInventoryReport {
    if ($InternalWorker.IsPresent) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($CredentialPath)) {
            if (-not (Test-Path -LiteralPath $CredentialPath)) {
                $workerErrorResult = New-ErrorResult -ComputerName $InternalComputerName -ConnectionMode $InternalConnectionMode -ErrorMessage ("Файл учётных данных не найден: {0}" -f $CredentialPath)
    ...
    Write-Host 'Собираю сведения по сетевым адаптерам...' -ForegroundColor Green
    Write-Host "Режим подключения: $($authenticationContext.DisplayName)" -ForegroundColor DarkGreen
    Write-Host "Файл со списком серверов: $(Resolve-ProjectPath -Path $ComputerListPath)" -ForegroundColor DarkGreen
    Write-Host "Папка выгрузки: $resolvedOutputDirectory" -ForegroundColor DarkGreen

    if ($Parallel.IsPresent) {
        Write-Host ("Запрошен многопоточный режим. Версия PowerShell: {0}" -f $PSVersionTable.PSVersion.ToString()) -ForegroundColor DarkGreen
    }
    ...
    Write-Host "Готово. Успешно обработано серверов: $successCount" -ForegroundColor Green
    if ($errorCount -gt 0) {
        Write-Warning "С ошибками обработано серверов: $errorCount. Подробности смотрите в network_adapter_summary.csv."
    }
    if ($multipleGatewayServerCount -gt 0) {
        Write-Warning "Найдено серверов с несколькими default gateway: $multipleGatewayServerCount. Подробности смотрите в network_multiple_gateways.csv."
    }

    Write-Host "Сводный отчёт: $($exportResult.SummaryPath)" -ForegroundColor DarkGreen
    Write-Host "Детали адаптеров: $($exportResult.AdapterPath)" -ForegroundColor DarkGreen
    Write-Host "Шлюзы: $($exportResult.GatewayPath)" -ForegroundColor DarkGreen
    Write-Host "Серверы с несколькими шлюзами: $($exportResult.MultipleGatewayPath)" -ForegroundColor DarkGreen
    Write-Host "Старые адаптеры: $($exportResult.LegacyPath)" -ForegroundColor DarkGreen
    Write-Host "Полный JSON: $($exportResult.JsonPath)" -ForegroundColor DarkGreen
}
